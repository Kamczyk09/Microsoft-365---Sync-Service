...
class OneDriveSyncCore:
    ...
    def sync_user(self, user_id: str):
        ...
        base_dir = os.path.join(BASE_ROOT, user_id, "onedrive")
...
def main():
    ...
    if args.authenticate:
        user_id = auth.device_code_flow()
        log(f"Authenticated {user_id}")
        return

    if args.sync:
        user_id = args.sync
        log(f"Starting sync for {user_id}")
        core.sync_user(user_id)
        return
